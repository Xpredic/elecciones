...
def cargar_parametros(carpeta, tipo):
    ...
    return pd.DataFrame(todos_parametros)
...
def proyectar_votos(df_ubigeo, candidato):
    ...
    actas_actual = ultimo['actasContabilizadas']
    
    if len(df_cand) >= 2:
        votos_anteriores = df_cand.iloc[-2]['total_votos_validos']
        actas_anteriores = df_cand.iloc[-2]['actasContabilizadas']
        delta_votos = votos_actuales - votos_anteriores
        delta_actas = actas_actual - actas_anteriores
        velocidad = delta_votos / delta_actas if delta_actas > 0 else votos_actuales / actas_actual if actas_actual > 0 else 0
    else:
        velocidad = votos_actuales / actas_actual if actas_actual > 0 else 0
    
    actas_pendientes = 100 - actas_actual
    votos_proyectados_lineal = votos_actuales + (velocidad * actas_pendientes)
    
    participacion = ultimo['participacionCiudadana']
...
df_parametros_globales = cargar_parametros(CARPETA_PARAMETROS_GLOBALES, "Parámetros globales")
...
print(f"   - parametros_globales.csv ({len(df_parametros_globales)} registros)")
